...
npx knip --version 2>/dev/null || echo "INSTALL_NEEDED"
```

If knip is not installed, install it temporarily:
```bash
npx knip --reporter json 2>/dev/null | head -2000
```
...
- `files`: Entirely unused files (highest confidence)
...
**Laravel-specific:** Also check for:
```bash
# Unused routes (defined but controller method missing or never hit)
grep -rn "Route::" routes/ 2>/dev/null | head -100

# Unused middleware
grep -rn "class.*Middleware" app/Http/Middleware/ 2>/dev/null | while read line; do
  CLASS=$(echo "$line" | grep -oP 'class \K\w+')
  REFS=$(grep -rn "$CLASS" app/ routes/ --include="*.php" | grep -v "class $CLASS" | wc -l)
  [ "$REFS" -eq 0 ] && echo "UNUSED_MIDDLEWARE:$line"
done

# Unused models (defined but never referenced)
for model in app/Models/*.php; do
  CLASS=$(basename "$model" .php)
  REFS=$(grep -rn "\b$CLASS\b" app/ routes/ database/ --include="*.php" | grep -v "class $CLASS" | wc -l)
  [ "$REFS" -le 1 ] && echo "MAYBE_UNUSED_MODEL:$model"
done

# Unused Blade views
for blade in $(find resources/views -name "*.blade.php"); do
  VIEW=$(echo "$blade" | sed 's|resources/views/||;s|\.blade\.php||;s|/|.|g')
  REFS=$(grep -rn "'$VIEW'" app/ routes/ resources/ --include="*.php" --include="*.blade.php" | wc -l)
...
- **Framework convention files:** Next.js pages/, app/, Laravel routes/,
  migrations, seeders, factories, middleware registered in Kernel
