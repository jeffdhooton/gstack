...
| `/notetaker` | **Workflow Observer** | Silently journals every tool call via a global PostToolUse hook. Run `/notetaker patterns` to spot repeatable workflows across sessions, then `/notetaker draft` to scaffold a new skill from an observed pattern. `/learn` captures facts; `/notetaker` captures workflows. |
