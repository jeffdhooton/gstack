...
The fix loop uses HTML previews for any target mockups — no external API needed.
...
For findings involving visual layout, hierarchy, or spacing (not just a CSS value fix), generate a target mockup as an HTML preview page showing what the corrected version should look like. Save to `$REPORT_DIR/screenshots/finding-NNN-target.html` and open it for the user.

Show the user: "Here's the current state (screenshot) and here's what it should look like (preview). Now I'll fix the source to match."

Skip for trivial CSS fixes (wrong hex color, missing padding value). Use it for findings where the intended design isn't obvious from the description alone.
...
2. If target mockup HTML previews were generated during the fix loop, compare the after-screenshot against the target preview visually. Include pass/fail in the report.
