...
version: 2.0.0
description: |
  Plan stage: Office Hours — two modes. Startup mode: six forcing questions that expose
...
## Phase 1: Context Gathering (SILENT — do not ask questions yet)

Understand the project deeply BEFORE asking anything. The goal is to arrive at Phase 2 already knowing what the codebase reveals, so you never ask the user something the code already answers.

```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
```

1. Read `CLAUDE.md`, `TODOS.md`, `README.md`, `package.json` / `Cargo.toml` / `go.mod` (if they exist).
...
5. **Build a context brief** (internal, do not show to user). Write down:
   - What the project does (from README/CLAUDE.md)
   - Current state (what's built, what's recent from git log)
   - Tech stack (from deps/config files)
   - Who uses it (from docs, landing page, README)
   - What problems are known (from TODOS.md, issues, recent commits)
   - What the user is likely asking about (from their prompt + recent git activity)

   **This context brief drives smart-skipping in Phase 2.** If the code already reveals the answer to a Phase 2 question, state what you found and ask only for confirmation or correction — don't ask the question from scratch.

6. **Single opening question** — combine goal detection and context confirmation into ONE AskUserQuestion:

   > Here's what I see: {2-3 sentence summary from the context brief — what the project is, what's recently changed, and what you think they want to discuss}.
   >
...
**CRITICAL: Use what you already know.** Before asking ANY question, check if the Phase 1 context brief already provides the answer (from README, CLAUDE.md, TODOS.md, git history, codebase). If it does:
- DO NOT ask the question from scratch
- Instead, state what you found and ask for confirmation: "From reading your codebase, it looks like [observation]. Is that right, or am I missing something?"
- Only push deeper if the codebase evidence is insufficient

**Never ask questions the code answers.** If you can read the tech stack from package.json, don't ask "what's your tech stack?" If git log shows the project is 2 weeks old with 30 commits, don't ask "how far along are you?" If README describes the target user, don't ask "who is this for?" from scratch.
...
**Escape hatch:** If the user expresses impatience ("just do it," "skip the questions," "I already told you," "you could read that from the code"):
- Immediately respect it. Proceed to Phase 3 with what you have. Do NOT push back or ask "just two more."
- Use the Phase 1 context brief to fill gaps. If the code reveals what the user would have answered, fill it in yourself.
- Still run Phase 3 (Premise Challenge) and Phase 4 (Alternatives) — but base them on codebase evidence + what the user already said, not on questions you didn't get to ask.
...
**Escape hatch:** If the user says "just do it," expresses impatience, or provides a fully formed plan → immediately proceed to Phase 3 with what you have. Use Phase 1 codebase context to fill gaps. Do NOT push back.
...
Based on the founder signals from Phase 4.5, give a brief, genuine closing that validates what was strong about their thinking. No pitches, no asks — just reflect back what you observed and encourage them to keep going.
